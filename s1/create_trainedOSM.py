...
parser.add_argument(
    "--model_name", type=str, default=None, help="name of the trained model"
)
...
parser.add_argument(
    ...
    type=int,
    default=0,
    help="num of warmup epochs before the architecture search",
)
...
def train_func(
    ...
    for step, (inputs, targets) in enumerate(data_loader):
        ...
        if random:
            ...
            if discrete:
                discrete_alphas = utils.discretize(
                    alphas=rnd, arch_genotype=model.genotype()
                )
        ...
        if step % print_freq == 0 or step + 1 == len(data_loader):
            logging.info(
                f"scheduler: {scheduler.get_last_lr()[0]} with random: {random}, train_discrete: {discrete}"
            )
            Sstr = (
                "*TRAIN* "
                + time_string()
                + " [{:03d}/{:03d}]".format(step, len(data_loader))
            )
...
def main(args):
    ...
    optimizer, _, criterion = get_optim_scheduler(
        parameters=model.parameters(), config=config
    )
    ...
    for epoch in range(total_epochs):
        logging.info("=" * 100)
        logging.info(
            f"[INFO] Epoch ({epoch + 1}/{total_epochs}) with LR {scheduler.get_last_lr()[0]}"
        )

        # TRAINING ONCE ON TRAINING SET with RANDOM ARCHITECTURE VALUES
        logging.info("TRAINING")
        train_start = time.time()
        losses, top1, top5 = train_func(
        ...
        logging.info(
            f"[INFO] Losses: {losses:.5f}, top1: {top1:.5f}, top5: {top5:.5f} finished in {(time.time()-train_start)/60:.3f} minutes"
        )
        scheduler.step()
