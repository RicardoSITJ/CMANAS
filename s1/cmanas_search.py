...
parser.add_argument(
    ...
    type=int,
    default=0,
    help="What alternate split to use for CIFAR100",
)
...
parser.add_argument(
    "--data_dir", type=str, default="../data", help="location of the data corpus"
)
...
parser.add_argument(
    ...
    type=int,
    default=1,
    help="number of trained models used for search",
)
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Whether to use discretization during training",
)
parser.add_argument(
...
parser.add_argument(
    ...
    type=int,
    default=0,
    help="num of epochs for training per epoch of CMAES",
)
...
def evaluate(
    ...
    assert model.check_alphas(
        alphas
    ), "Given alphas has not been copied successfully to the model"
    ...
    if series is not None:
        arch_loss_list, arch_top1_list, arch_top5_list = (
            series["arch_loss"],
            series["arch_top1"],
            series["arch_top5"],
        )
        if pop_flag:
            ...
            logging.info(
                f'[INFO] Mean architecture already evaluated in generation {series["generation"]}'
            )
    else:
        ...
        discrete_alphas = utils.discretize(
            alphas=alphas, arch_genotype=model.genotype()
        )
        ...
        for trained_model in trained_models:
            ...
            # def eval_arch(data_loader, model, criterion):
            arch_loss, arch_top1, arch_top5 = eval_arch(
                data_loader=valid_loader, model=model, criterion=criterion
            )

            if pop_flag:
                logging.info(
                    f"[INFO] Evaluating ({ind+1:03d}/{pop_size:03d}) Losses: {arch_loss:.5f}, top1: {arch_top1:.5f}, top5: {arch_top5:.5f} finished in {(time.time()-pre_start)/60:.3f} minutes"
                )
            else:
                logging.info(
                    f"[INFO] Evaluating (mean architecture) Losses: {arch_loss:.5f}, top1: {arch_top1:.5f}, top5: {arch_top5:.5f} finished in {(time.time()-pre_start)/60:.3f} minutes"
                )
        ...
        d_tmp = {
            ...
            "arch_loss": arch_loss_list,
            "arch_top1": arch_top1_list,
            "arch_score": arch_top1_list[0],
    ...
    logging.info(
        f"[INFO] architecture stats: Loss: {arch_loss_list} with top5: {arch_top5_list}"
    )
...
def main(args):
    ...
    if args.pre_tr_model == "None":
        ...
        trained_models = [
            os.path.join(args.tr_model_dir, pt_file)
            for pt_file in tmp_files
            if "pt" in pt_file
        ]
    ...
    if args.cutout:
        train_data, valid_data, xshape, num_classes = get_datasets(
            name=args.dataset, root=args.data_dir, cutout=args.cutout_length
        )
    else:
        train_data, valid_data, xshape, num_classes = get_datasets(
            name=args.dataset, root=args.data_dir, cutout=-1
        )
    ...
    if args.dataset == "cifar100" and (args.alt_cifar100_split == 1):
        ...
        logging.info(
            f"cifar100_split.train: {len(cifar100_split.train)}, cifar100_split.valid: {len(cifar100_split.valid)}"
        )
    ...
    config = load_config(
        ...
        extra={"class_num": num_classes, "xshape": xshape},
        logger=None,
    )
    ...
    cmaes_optimizer = CMAES(
        mean=np.zeros(
            model.arch_parameters()[0].view(-1).shape[0] * 2, dtype=np.float32
        ),
        sigma=1.3,
        bounds=None,
        seed=args.seed,
    ...
    for epoch in range(total_epochs + 1):
        ...
        if epoch < total_epochs:
            ...
            if epoch < total_epochs:
                logging.info(
                    "[INFO] Epoch ({}/{})Evaluation finished in {:.3f} minutes".format(
                        epoch + 1, total_epochs, (time.time() - eval_start) / 60
                    )
                )
        ...
        df, mean_loss, mean_top1, mean_top5 = evaluate(
            valid_loader=valid_loader,
            criterion=criterion,
            df=df,
            alphas=tx,
        ...
        logging.info(
            f"[INFO] Score of the mean architecture, {mean_top1} finished in {(time.time()-mean_start)/60:.3f} minutes"
        )

        logging.info(
            f"[INFO] length of the main dataframe: {len(df)}, solutions: {len(solutions)}"
        )
        ...
        logging.info(
            f"[INFO] Mean architecture: {genotype_list[-1]} with top1: {mean_top1[0]:.3f}"
        )
        print(
            f"[INFO] Mean architecture: {genotype_list[-1]} with top1: {mean_top1[0]:.3f}"
        )
    ...
    logging.info(
        f"[INFO] Architecture search finished in {(time.time()-epoch_start) / 3600:.5f} hours"
    )
